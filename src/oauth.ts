/**
 * Small self-hosted OAuth 2.1 authorization server for music-mcp.
 *
 * It intentionally supports only public clients, Authorization Code + S256
 * PKCE, and one read scope. Codes and dynamic client registrations are kept in
 * memory; signed access tokens survive a process restart.
 */
...
function tokenKey(secret: string): Buffer {
  return crypto.createHash("sha256").update(`music-mcp-access-token\0${secret}`).digest();
}
...
function verifyAccessToken(token: string, options: OAuthSetupOptions, key: Buffer): AccessClaims | null {
  ...
  if (!safeEqualText(signaturePart, signPayload(payloadPart, key))) return null;
  try {
...
<title>${htmlEscape(serviceName)} · 授权</title>
<style>
...
.card{background:#fff;border:1px solid #f3dce9;border-radius:14px;padding:32px 28px;
      box-shadow:0 4px 20px rgba(180,120,160,.1);max-width:390px;width:100%}
h1{font-size:17px;font-weight:700;color:#3a3340;margin-bottom:6px}
p{font-size:13px;color:#8a7d8f;margin-bottom:18px;line-height:1.55}
.scope{font:12px ui-monospace,monospace;color:#8b8bef;background:#e9e9fe;border-radius:7px;padding:8px;margin-bottom:18px;word-break:break-all}
...
.err{margin-bottom:12px;font-size:13px;color:#c43e62;background:#fff0f4;padding:9px;border-radius:8px}
</style>
</head><body><div class="card">
  <h1>${htmlEscape(serviceName)}</h1>
  <p>请输入部署时设置的密码，授权此客户端只读访问音乐搜索与播放器。</p>
  ${error ? `<div class="err">${htmlEscape(error)}</div>` : ""}
  <div class="scope">resource: ${htmlEscape(fields.resource ?? "")}<br>scope: ${htmlEscape(fields.scope ?? "")}</div>
...
export function setupOAuth(app: Express, rawOptions: OAuthSetupOptions) {
  if (!rawOptions.password) {
    return (_req: Request, _res: Response, next: NextFunction) => next();
  }
  if (!rawOptions.tokenSecret) {
    throw new Error("MCP_AUTH_TOKEN_SECRET or MCP_AUTH_PASSWORD is required when OAuth is enabled.");
  }

  const options: OAuthSetupOptions = {
    ...rawOptions,
  ...
  app.get("/.well-known/oauth-authorization-server", (_req, res) => res.json(authorizationServerMeta));
  ...
  app.post("/oauth/register", express.json({ limit: "64kb" }), (req, res) => {
    ...
    const clientName = typeof body.client_name === "string"
      ? body.client_name.trim().slice(0, 120) || options.serviceName
      : options.serviceName;
  ...
  const validateAuthorization = (input: Record<string, unknown>) => {
    ...
    const codeChallengeMethod = asString(input.code_challenge_method) ?? "S256";
  ...
  app.post("/oauth/authorize", express.urlencoded({ extended: false, limit: "32kb" }), (req, res) => {
    ...
    if (attempt && attempt.resetAt > Date.now() && attempt.count >= 10) {
      ...
      res.status(429).send("Too many failed authorization attempts.");
      return;
    }
  ...
  app.post(
    ...
    (req, res) => {
      ...
      if (!resource || resource !== record.resource) {
        res.status(400).json({ error: asString(body.resource) ? "invalid_target" : "invalid_request" });
        return;
      }
      if (
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.setHeader(
      "WWW-Authenticate",
      `Bearer resource_metadata="${options.baseUrl}${metadataPath}", scope="${options.scope}"`
    );
